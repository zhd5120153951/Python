...
from marshmallow.validate import (
    URL, Email, Range, Length, Equal, Regexp, Predicate, NoneOf, OneOf, ContainsOnly)
...
class Query(BaseQuery):
    def soft_delete(self):
        return self.update({"delete-at": datetime.datetime.now()})
    ...
    def layui_paginate(self):
        return self.paginate(
            page=request.args.get('page', type=int), per_page=request.args.get('limit', type=int), error_out=False)
