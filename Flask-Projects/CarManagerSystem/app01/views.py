...
import datetime
import re
from app01.paginater import Paginater
import time
import random
...
def Park_discern(image):
        APP_ID = '28204164'
        API_KEY = 'RNSAnAfltTTI9auF5mZaTf3q'
        SECRET_KEY = 'OVLG3enKIsiHsn41iuxE7o7dL28baIOa'
        # 创建客户端对象
...
@login_required
@vip_valid
def car_find(request):
    return redirect('/car_find_paging/')
...
@login_required
@vip_valid
def car_find_paging(request):
    # 从URL中取参数page，这个参数与pageinanter.py生成的HTML代码片段有关
    car_id = request.GET.get('car_id')
    print(car_id)
    if car_id != None:
        car_info = models.Car_manage.objects.all().filter(carnum=car_id)
        # 对表中的数据进行切片，取出属于本页的记录
        return render(request, 'car_find.html', {'all_cars': car_info})
    else:
        return render(request, 'car_find.html')

@login_required
@vip_valid
def code(request):
    img = qrcode.make('https://baike.baidu.com/item/%E9%87%8D%E5%BA%86%E5%B8%82/436625?fromtitle=%E9%87%8D%E5%BA%86&fromid=23586&fr=aladdin')  # 传入网站计算出二维码图片字节数据
