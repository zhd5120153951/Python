...
def rec_pre_precessing(img, size=(48, 168)):  # 识别前处理
    ...
    # channel,height,width转为batch,channel,height,channel
...
def get_plate_result(img, session_rec):  # 识别后处理
    ...
    y_onnx_plate, y_onnx_color = session_rec.run([session_rec.get_outputs(
    )[0].name, session_rec.get_outputs()[1].name], {session_rec.get_inputs()[0].name: img})
...
def draw_result(orgimg, dict_list):
    ...
    for result in dict_list:
        ...
        cv2.rectangle(orgimg, (rect_area[0], rect_area[1]),
                      (rect_area[2], rect_area[3]), (255, 255, 0), 2)  # 画框
        if len(result) >= 1:
            orgimg = cv2ImgAddText(
                orgimg, result, rect_area[0]-height_area, rect_area[1]-height_area-10, (0, 255, 0), height_area)
    print(result_str)
    return orgimg
...
if __name__ == "__main__":
    ...
    parser.add_argument('--detect_model', type=str,
                        default=r'weights/plate_detect.onnx', help='model.pt path(s)')  # 检测模型
    parser.add_argument('--rec_model', type=str,
                        default='weights/plate_rec_color.onnx', help='model.pt path(s)')  # 识别模型
    parser.add_argument('--image_path', type=str,
                        default='imgs', help='source')
    ...
    session_detect = onnxruntime.InferenceSession(
        opt.detect_model, providers=providers)
    session_rec = onnxruntime.InferenceSession(
        opt.rec_model, providers=providers)
    ...
    for pic_ in file_list:
        ...
        # print(img.shape)
        y_onnx = session_detect.run([session_detect.get_outputs()[0].name], {
                                    session_detect.get_inputs()[0].name: img})[0]
